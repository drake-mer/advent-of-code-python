...
def test_data1():
    return [
        eval(l)
        for l in """[[[0,[4,5]],[0,0]],[[[4,5],[2,6]],[9,5]]]
[7,[[[3,7],[4,3]],[[6,3],[8,8]]]]
...
def test_data2():
    return [
        eval(l)
        for l in """[[[0,[5,8]],[[1,7],[9,6]]],[[4,[1,2]],[[1,4],2]]]
[[[5,[2,8]],4],[5,[[9,9],0]]]
[6,[[[6,2],[5,6]],[[7,6],[4,7]]]]
[[[6,[0,7]],[0,9]],[4,[9,[9,0]]]]
[[[7,[6,4]],[3,[1,3]]],[[[5,5],1],9]]
...
class Node(BinaryTree):
    ...
    def split(self):
        ...
        if isinstance(self.left, Leaf) and self.left.value > 9:
            self.left = Node(
                self.left.value // 2,
                self.left.value // 2 + self.left.value % 2,
                parent=self,
            )
            ## Split
            p = self.parent
        ...
        if isinstance(self.right, Leaf) and self.right.value > 9 and not self.splitted:
            self.right = Node(
                self.right.value // 2,
                self.right.value // 2 + self.right.value % 2,
                parent=self,
            )
            ## Split
            p = self.parent
...
def test_explode_is_single_step():
    assert (
        Node(*[[3, [2, [1, [7, 3]]]], [6, [5, [4, [3, 2]]]]]).explode().as_tuple()
        == Node(*[[3, [2, [8, 0]]], [9, [5, [4, [3, 2]]]]]).as_tuple()
    )


def test_split():
    assert (
        Node(*[[[[0, 7], 4], [15, [0, 13]]], [1, 1]]).split().as_tuple()
        == Node(*[[[[0, 7], 4], [[7, 8], [0, 13]]], [1, 1]]).as_tuple()
    )
    assert (
        Node(*[[[[0, 7], 4], [[7, 8], [0, 13]]], [1, 1]]).split().as_tuple()
        == Node(*[[[[0, 7], 4], [[7, 8], [0, [6, 7]]]], [1, 1]]).as_tuple()
    )
...
@pytest.mark.parametrize(
    "d",
    [Node(*[[[[7, 7], [7, 8]], [[9, 5], [8, 0]]], [[[9, [5, 5]], 20], [8, [9, 0]]]])],
)
