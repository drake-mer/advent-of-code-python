...
def make_code(digits):
    ...
    mapped_digits = {UNIQUE_SIZE[len(d)]: d for d in digits if len(d) in UNIQUE_SIZE}
    ...
    dee = "".join(l for l in four if output_map.get(l) not in ("f", "b", "c"))
    assert len(dee) == 1
    output_map[dee] = "d"

    aaa = "".join(l for l in seven if output_map.get(l) not in ("c", "f"))
    assert len(aaa) == 1
    output_map[aaa] = "a"
